    ...
    async fetchItemDetail(title) {
      ...
      try {
        const res = await uni.request({
          url: this.apiBase + '/detail/' + encodeURIComponent(title),
          method: 'GET'
        });
        
        if (res[1].data && res[1].data.success && res[1].data.data) {
          this.itemData = res[1].data.data;
          // 设置页面标题
          uni.setNavigationBarTitle({
            title: this.itemData.title.split('：')[0] || '非遗详情'
          });
        } else {
          console.error('获取详细内容失败:', res);
          this.itemData = null;
        }
      } catch (error) {
