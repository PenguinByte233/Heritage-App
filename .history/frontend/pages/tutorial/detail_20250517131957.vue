...
export default {
  data() {
    return {
      ...
      isLoading: true,
      itemData: null,
      baseUrl: 'http://localhost:3001', // 修改为正确的端口3001
      errorMessage: ''
    }
  },
  computed: {
  ...
  methods: {
    ...
    fetchItemById(id) {
      ...
      // 构建完整URL，修改为正确的API路径
      const url = `${this.baseUrl}/api/learn/items/${id}`;
      ...
      uni.request({
        ...
        method: 'GET',
        success: (res) => {
          console.log('根据ID获取详情响应:', res); // 调试用
    ...
    async fetchItemDetail(title) {
      ...
      try {
        // 直接使用未编码的中文标题
        const apiUrl = `${this.baseUrl}/api/learn/detail/${title}`;
