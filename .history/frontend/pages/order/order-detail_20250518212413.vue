...
export default {
  ...
  methods: {
    ...
    fetchOrderDetail() {
      ...
      // 调用API获取订单详情
      uni.request({
        url: `${this.apiBaseUrl}/orders/${this.orderId}`,
        method: 'GET',
        header: {
      ...
      uni.showModal({
        ...
        success: (res) => {
          if (res.confirm) {
            // 调用取消订单API
            uni.request({
              url: `${this.apiBaseUrl}/orders/${orderId}/cancel`,
              method: 'PUT',
              header: {
                'Authorization': `Bearer ${uni.getStorageSync('token')}`
              },
    ...
    remindShipping(orderId) {
      // 调用提醒发货API
      uni.request({
        url: `${this.apiBaseUrl}/orders/${orderId}/remind`,
        method: 'POST',
        header: {
          'Authorization': `Bearer ${uni.getStorageSync('token')}`
        },
      ...
      uni.showModal({
        ...
        success: (res) => {
          if (res.confirm) {
            // 调用确认收货API
            uni.request({
              url: `${this.apiBaseUrl}/orders/${orderId}/confirm`,
              method: 'PUT',
              header: {
                'Authorization': `Bearer ${uni.getStorageSync('token')}`
              },
