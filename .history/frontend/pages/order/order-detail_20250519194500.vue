  ...
  <view class="container">
    ...
    <scroll-view class="content" scroll-y v-if="!isLoading && order">
      ...
      <view class="footer-actions">
        ...
        <!-- 订单导航按钮 - 移到footer-actions内，确保能够正确显示 -->
        <view class="order-navigation" v-if="prevOrderId || nextOrderId">
          <view class="navigation-title">其他订单</view>
          <view class="navigation-buttons">
            <view 
              class="nav-button prev" 
              v-if="prevOrderId" 
              @tap="navigateToOrder(prevOrderId)"
            >
              上一订单
            </view>
            <view 
              ...
              v-if="nextOrderId" 
              @tap="navigateToOrder(nextOrderId)"
            >
              下一订单
            </view>
          </view>
...
export default {
  data() {
    return {
      ...
      orderId: '',
      order: null,
      prevOrderId: null, // 上一个订单的ID
      nextOrderId: null, // 下一个订单的ID
  ...
  methods: {
          ...
          if (res.data && res.data.status === 'success') {
            ...
            if (!orderData) {
              ...
              if (orderList && orderList.length > 0) {
                ...
                // 更新URL参数，以便刷新页面时能正确加载该订单
                this.orderId = orderList[0].id.toString();
                
                // 如果有多个订单，设置导航
                if (orderList.length > 1) {
                  this.nextOrderId = orderList[1].id;
                  console.log('设置下一订单ID:', this.nextOrderId);
                }
            ...
            console.log('选中的订单数据:', this.order);
            
            // 添加订单切换功能
            if (orderList.length > 1) {
              // 找到当前订单在列表中的索引
              const currentIndex = orderList.findIndex(o => o.id.toString() === this.orderId);
              this.prevOrderId = currentIndex > 0 ? orderList[currentIndex - 1].id : null;
              this.nextOrderId = currentIndex < orderList.length - 1 ? orderList[currentIndex + 1].id : null;
              
              console.log('订单导航:', {
                总数: orderList.length,
                当前索引: currentIndex,
                上一订单: this.prevOrderId,
                下一订单: this.nextOrderId
              });
            } else {
              this.prevOrderId = null;
    ...
    navigateToOrder(orderId) {
      ...
      console.log('导航到订单:', orderId);
      
      // 直接设置orderId并刷新数据，避免页面跳转
      this.orderId = orderId.toString();
      console.log('已设置新的订单ID:', this.orderId);
      this.isLoading = true;
      this.fetchOrderDetail();
