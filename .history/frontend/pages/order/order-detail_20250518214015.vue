...
export default {
  ...
  methods: {
    ...
    fetchOrderDetail() {
      ...
      // 调用API获取订单详情
      uni.request({
        url: `${this.apiBaseUrl}/orders/user/${userId}`,
        method: 'GET',
        header: {
      ...
      uni.showModal({
        ...
        success: (res) => {
          if (res.confirm) {
            ...
            // 调用取消订单API
            uni.request({
              url: `${this.apiBaseUrl}/orders/user/${userId}/cancel/${orderId}`,
              method: 'PUT',
              header: {
                'Authorization': `Bearer ${token}`
              },
    ...
    remindShipping(orderId) {
      ...
      // 调用提醒发货API
      uni.request({
        url: `${this.apiBaseUrl}/orders/user/${userId}/remind/${orderId}`,
        method: 'POST',
        header: {
          'Authorization': `Bearer ${token}`
        },
      ...
      uni.showModal({
        ...
        success: (res) => {
          if (res.confirm) {
            ...
            // 调用确认收货API
            uni.request({
              url: `${this.apiBaseUrl}/orders/user/${userId}/confirm/${orderId}`,
              method: 'PUT',
              header: {
                'Authorization': `Bearer ${token}`
              },
      ...
      uni.showModal({
        ...
        success: (res) => {
          if (res.confirm) {
            ...
            // 调用删除订单API
            uni.request({
              url: `${this.apiBaseUrl}/orders/user/${userId}/delete/${orderId}`,
              method: 'DELETE',
              header: {
                'Authorization': `Bearer ${token}`
              },
