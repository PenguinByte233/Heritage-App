...
export default {
  ...
  methods: {
    ...
    async testAPI() {
      ...
      try {
        // 测试首页资源 API
        console.log('测试 getHomeResources API...');
        const homeResponse = await uni.request({
          url: 'http://localhost:3001/api/home/resources',
          method: 'GET'
        });
        console.log('首页资源 API 原始响应:', homeResponse);
        
        // 测试地图资源 API
        console.log('测试 getMapResources API...');
        const mapResponse = await uni.request({
          url: 'http://localhost:3001/api/home/map-resources',
          method: 'GET'
        });
        console.log('地图资源 API 原始响应:', mapResponse);
        
        // 测试封装的 API 函数
        console.log('测试封装的 getHomeResources 函数...');
        const homeData = await getHomeResources();
        console.log('封装的首页资源函数返回:', homeData);
        
        console.log('测试封装的 getMapResources 函数...');
        const mapData = await getMapResources();
        console.log('封装的地图资源函数返回:', mapData);
    ...
    navigateToLearnCategory(item) {
      ...
      if (learnCategories.includes(item.title)) {
        // 如果是已定义的分类，直接导航到学习页面并传递标题参数
        uni.switchTab({
          url: '/pages/learn/learn',
          success: () => {
            // 使用全局变量存储选中的分类，以便在learn.vue中获取
