...
<script>
import { getHomeResources } from '@/api/api.js';
      ...
      try {
        ...
        if (response && response.success && response.data) {
          ...
          if (response.data.projectItems && response.data.projectItems.length > 0) {
            this.projectItems = response.data.projectItems;
          } else {
            // 使用默认数据
            this.projectItems = [
              {
