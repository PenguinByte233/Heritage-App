...
export default {
  data() {
    return {
      ...
      isLoading: false,
      baseUrl: 'http://localhost:3000', // 添加完整的基础URL
      errorMessage: ''
    }
  },
  created() {
  ...
  methods: {
    ...
    async fetchCategoryItems(category) {
      ...
      try {
        // 修复URL问题，确保有完整的基础URL，并正确编码参数
        const encodedCategory = encodeURIComponent(category);
        // 这里使用完整的基础URL，避免相对路径问题
        const apiUrl = `${this.baseUrl}/api/learn/category/${encodedCategory}`;
        
        console.log('请求URL:', apiUrl); // 调试用，查看实际请求的URL
        ...
        if (response.statusCode === 200 && response.data && response.data.success) {
          ...
          // 调试输出，检查返回的数据
          console.log('获取到的分类数据:', JSON.stringify(this.currentCategoryData));
        } else {
    ...
    navigateToDetail(item) {
      ...
      // 直接使用ID参数而非title，因为ID更可靠，不会有中文编码问题
      if (item.id) {
        uni.navigateTo({
        ...
        // 如果没有ID，才使用title参数，但需要特别注意编码
        const encodedTitle = encodeURIComponent(item.title);
        console.log('跳转到详情页，标题参数:', encodedTitle);
        
        uni.navigateTo({
          url: `/pages/tutorial/detail?title=${encodedTitle}`,
          fail: (err) => {
            console.error('导航失败:', err);
            uni.showToast({
              title: '页面跳转失败',
