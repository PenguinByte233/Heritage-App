  ...
  methods: {
    ...
    async fetchCategoryData() {
      ...
      try {
        ...
        // 直接使用中文参数
        const url = `${this.baseUrl}/api/learn/category/${categoryName}`;
        ...
        if (response.statusCode === 200 && response.data && response.data.success) {
          ...
          // 使用API返回的数据更新界面
          if (this.apiData && this.apiData.subcategories) {
            // 更新推荐教程列表
            let newTutorials = [];
            
            // 遍历所有子分类，收集其中的项目
            this.apiData.subcategories.forEach(subcategory => {
            ...
            if (categoryName === '传统表演艺术' && this.apiData.subcategories.length > 0) {
              const performanceSubcategories = this.apiData.subcategories.map((subcat, index) => {
