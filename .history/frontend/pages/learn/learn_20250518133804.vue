      ...
      >
        ...
        >
          <swiper-item v-for="(img, imgIndex) in item.images.slice(0, 3)" :key="imgIndex">
            <image class="subcategory-image" :src="img" mode="aspectFill"></image>
          </swiper-item>
        </swiper>
...
export default {
  ...
  methods: {
    async fetchSubcategories(title) {
      ...
      try {
        ...
        if (response.status === 'success' && response.data) {
          ...
          console.log('获取到的小标题数据:', this.subcategories);
        } else {
          uni.showToast({
            title: '获取数据失败',
            icon: 'none'
    ...
    getImageCountText(item) {
      ...
      const currentIndex = this.currentSwiperIndex[item.index] || 0;
      ...
      if (totalImages > 3) {
        ...
        return `${currentIndex + 1}/${displayCount}`;
      }
    }
  }
};
