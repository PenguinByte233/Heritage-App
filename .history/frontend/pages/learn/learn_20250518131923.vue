...
.container {
  padding: 0;
  background-color: #f0e6d2; /* 统一使用顶部标题的背景色 */
  min-height: 100vh;
}
...
.nav-tabs {
  display: flex;
  white-space: nowrap;
  background-color: #f7f0e3; /* 稍微浅一点的背景色，与主背景区分 */
  padding: 0;
  border-bottom: 1px solid #e6dcc8;
  position: relative;
  z-index: 10;
  box-shadow: 0 2rpx 6rpx rgba(109, 65, 38, 0.05);
}
...
.subcategory-item {
  ...
  overflow: hidden;
  box-shadow: 0 6rpx 16rpx rgba(109, 65, 38, 0.12); /* 阴影颜色与主题一致 */
  transition: transform 0.3s ease, box-shadow 0.3s ease;
}

.subcategory-item:active {
  transform: scale(0.98); /* 点击时的缩小效果 */
  box-shadow: 0 3rpx 8rpx rgba(109, 65, 38, 0.1);
}
