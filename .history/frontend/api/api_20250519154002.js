...
export function getHomeResources() {
  // 直接返回完整的请求结果，不需要处理
  return uni.request({
    url: `${BASE_URL}/home/resources`,
    method: 'GET'
  }).then(res => {
    if (res.statusCode >= 200 && res.statusCode < 300) {
      return res.data;
    } else {
      console.error('获取首页资源失败:', res);
      throw res;
    }
  }).catch(err => {
    console.error('请求首页资源出错:', err);
    throw err;
  });
}
...
export function getMapResources() {
  // 直接返回完整的请求结果，不需要处理
  return uni.request({
    url: `${BASE_URL}/home/map-resources`,
    method: 'GET'
  }).then(res => {
    if (res.statusCode >= 200 && res.statusCode < 300) {
      return res.data;
    } else {
      console.error('获取地图资源失败:', res);
      throw res;
    }
  }).catch(err => {
    console.error('请求地图资源出错:', err);
    throw err;
  });
} 