...
// 定义获取基础URL的函数，根据不同环境返回不同URL
const getBaseUrl = () => {
  // 获取当前运行环境
  const accountInfo = uni.getAccountInfoSync();
  const envType = accountInfo.miniProgram ? accountInfo.miniProgram.envVersion : 'develop';
  ...
  switch(envType) {
    case 'develop': // 开发环境
      ...
      // #ifdef MP-WEIXIN
      return 'http://192.168.1.100:3001'; // 替换为您的实际局域网IP
      // #endif
    ...
    case 'trial': // 体验版
      return 'http://your-staging-server.com';
...
// 动态获取BASE_URL和SERVER_URL
const SERVER_URL = getBaseUrl();
const BASE_URL = `${SERVER_URL}/api`;

console.log('当前使用的服务器URL:', SERVER_URL);
console.log('当前使用的API基础URL:', BASE_URL);
