...
// 获取所有分类及子分类
app.get('/api/learn/categories', async (req, res) => {
  try {
...
// 根据ID获取详细内容
app.get('/api/learn/items/:id', async (req, res) => {
  try {
    const { id } = req.params;
    ...
    let content = '';
    try {
      const filePath = path.join(__dirname, item.content_file_path);
...
// 根据大标题获取卡片信息
app.get('/api/learn/category/:category', async (req, res) => {
  try {
    // 获取并解码参数
...
// 根据标题获取详细内容
app.get('/api/learn/detail/:title', async (req, res) => {
  try {
    // 获取并解码参数
