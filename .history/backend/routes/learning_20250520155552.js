...
router.get('/content', async (req, res) => {
  try {
    ...
    if (item.all_content_urls) {
      // 尝试不同的分隔符处理方式
      // 递归分割处理 - 先采用逗号分隔内容URL
      const processContentUrls = (urlsString) => {
        console.log(`处理内容URL字符串: "${urlsString}"`);
      ...
      // 处理复杂的URL情况
      const contentUrls = processContentUrls(item.all_content_urls);
      console.log(`分析出 ${contentUrls.length} 个内容文件URL`, contentUrls);
      
      // 对每个内容URL单独处理
      for (let i = 0; i < contentUrls.length; i++) {
        const contentUrl = contentUrls[i];
        console.log(`处理内容文件 ${i+1}/${contentUrls.length}: "${contentUrl}"`);
        
        try {
          const contentPath = path.join(process.cwd(), contentUrl);
          console.log(`尝试读取文件: ${contentPath}`);
          
          if (fs.existsSync(contentPath)) {
            // 文件存在，读取内容
            const content = fs.readFileSync(contentPath, 'utf8');
            
            // 提取文件名作为标题，从URL中获取最后一个路径段
            const pathSegments = contentUrl.split('/');
            ...
            console.log(`  成功读取文件 "${title}"，内容长度: ${content.length} 字符`);
          } else {
